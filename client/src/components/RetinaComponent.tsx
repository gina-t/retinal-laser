...
function RetinaComponent() {
  ...
  const restartAnimation = () => {
    // Reset all spots to invisible
    gsap.set('#laser-spots .spot', { opacity: 0 });
    // Trigger animation to run again
    setAnimationCount(prev => prev + 1);
  };

  // Scroll trigger for mobile devices
  useEffect(() => {
    if (isMobile) {
      const currentRef = retinaRef.current;
    ...
    return () => {
      if (currentRef) { // Use stored reference in cleanup
  ...
  useGSAP(
    ...
    { scope: retinaRef, dependencies: [animationCount] }
  );
  ...
  return (
    <div>
      ...
      {/* Restart button - only visible on desktop */}
      {!isMobile && (
        <button 
          onClick={restartAnimation}
          className="absolute bottom-4 right-4 bg-sky-400 hover:text-white text-gray-900 px-4 py-2 rounded-md shadow-md transition-colors"
          aria-label="Restart animation"
        >
          Restart Laser Simulation
        </button>
      )}
    </div>
